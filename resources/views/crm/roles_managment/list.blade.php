...
@section('contenido_inmobiliaria')
    <div class="card">
        <div class="header">
            <h2><i class="material-icons">list</i> Roles</h2>
        ...
        <div class="body">
          <table id="roles_managment_table" class="table table-bordered table-striped table-hover dataTable js-exportable" style="width: 100%;">
              <thead>
              <tr>
                  <th>
...
@section('pages_js_files')
    ...
    <script>
        ...
        $(document).ready(function () {
          if($('#roles_managment_table').size() > 0){

            $('#roles_managment_table').DataTable(
              $.extend(true, {}, $.InmobiliariaSoft.options.DATATABLE_TEMPLATE,{
                  ajax: '/admin/roles_managment',
                  type: 'GET',
                  columns: [
                    {
                      data: function (data) {
                    ...
                    { data: 'description', name: 'description' },
                    {
                      data: function (data) {
                        ...
                        if (data.view === true) {
                          buttons += '<a href="/admin/roles_managment/role/' + data.id + '" target="_blank" class="btn btn-info btn-xs waves-effect" ' +
                            '  data-toggle="tooltip" data-original-title="Consultar"><i class="material-icons">remove_red_eye</i></a>';
                        }
                        if (data.edit === true) {
                          buttons +=
                            '<a href="/admin/roles_managment/role/' + data.id + '/edit" target="_blank" class="btn btn-warning btn-xs waves-effect edit-brand" ' +
