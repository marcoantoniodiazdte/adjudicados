...
@section('contenido_inmobiliaria')
    <div class="card">
        <div class="header">
            <h2><i class="material-icons">list</i> Proyectos Financiados</h2>
            <ul class="header-dropdown">
                @can('create.propiedades')
                    <li>
                        <a href="{{route('proyectosFinanciados.create')}}" data-toggle="tooltip" data-original-title="Crear">
          ...
          if($('#proyectos_table').size() > 0){
            ...
            $('#proyectos_table').DataTable(
                $.extend(true, {}, $.InmobiliariaSoft.options.DATATABLE_TEMPLATE,{
                    ajax: '/admin/proyectosFinanciados',
                    type: 'GET',
                    columns: [
                        {
                            data: function (data) {
                        ...
                        { data: 'name', name: 'name' },
                        {
                            data: function (data) {
                                ...
                                if (data.edit === true) {
                                    buttons +=
                                        '<a href="/admin/proyectosFinanciados/' + data.id + '/edit" target="_blank" class="btn btn-warning btn-xs waves-effect edit-brand" ' +
