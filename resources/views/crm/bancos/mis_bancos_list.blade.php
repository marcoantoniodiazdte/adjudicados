...
@section('contenido_inmobiliaria')
    <div class="card">
        <div class="header">
            <h2><i class="material-icons">list</i>Mis Bancos</h2>
        ...
        <div class="body">
            <div class="body">
                <table id="inmobiliaria_bancos_table" class="table table-bordered table-striped table-hover dataTable js-exportable" style="width: 100%;">
                    <thead>
                    <tr>
                        <th>
                        ...
                        <th class="exportar">Banco</th>
                        <th class="exportar">Número de Cuenta</th>
                        <th class="exportar">Tipo Cuenta</th>
                        <th class="exportar">Tipos Moneda</th>
...
@section('pages_js_files')
    ...
    <script>
        $(document).ready(function () {
            if($('#inmobiliaria_bancos_table').size() > 0 ){
                $('#inmobiliaria_bancos_table').DataTable(
                    $.extend(true, {}, $.InmobiliariaSoft.options.DATATABLE_TEMPLATE,{
                            ajax: '{!! route('admin_mis_bancos_managment') !!}',
                            type: 'GET',
                            columns: [
                                {
                                    data: function (data) {
                                ...
                                { data: 'name', name: 'name' },
                                { data: 'pivot.numero_cuenta', name: 'pivot.numero_cuenta' },
                                { data: 'pivot.tipo_cuenta', name: 'pivot.tipo_cuenta',
                                  render: function (value, type, obj, meta) {

                                    return value.toUpperCase()+ " " + obj.pivot.id;
                                  }
                                },
                                { data: 'pivot.tipo_moneda', name: 'pivot.tipo_moneda' },

                                {
                                    data: function (data) {
                                        ...
                                        if (data.edit === true) {
                                            buttons +=
                                                '<a href="/admin/bancos/mis_bancos/' + data.pivot.id + '/edit" target="_blank" class="btn btn-warning btn-xs waves-effect edit-brand" ' +
