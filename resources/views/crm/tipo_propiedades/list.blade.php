...
@section('contenido_inmobiliaria')
    <div class="card">
        <div class="header">
            <h2><i class="material-icons">list</i>Tipo de Propiedades</h2>
            <ul class="header-dropdown">
                @can('create.propiedades')
                    <li>
                        <a href="{{route('tipoPropiedades.create')}}" data-toggle="tooltip" data-original-title="Create">
        ...
        <div class="body">
           ...
           <table id="obras_table" class="table table-bordered table-striped table-hover dataTable js-exportable" style="width: 100%;">
                <thead>
                <tr>
...
@section('pages_js_files')
    ...
    <script>
        if($('#obras_table').size() > 0){
    
        $('#obras_table').DataTable(
            $.extend(true, {}, $.InmobiliariaSoft.options.DATATABLE_TEMPLATE,{
                ajax: '/admin/tipoPropiedades',
                type: 'GET',
                columns: [
                    {
                        data: function (data) {
                    ...
                    { data: 'name', name: 'name' },
                    {
                        data: function (data) {
                            ...
                            // if (data.view === true) {
                            //     buttons += '<a href="/admin/tipoPropiedades/' + data.id + '" target="_blank" class="btn btn-info btn-xs waves-effect" ' +
                            ...
                            if (data.edit === true) {
                                buttons +=
                                    '<a href="/admin/tipoPropiedades/' + data.id + '/edit" target="_blank" class="btn btn-warning btn-xs waves-effect edit-brand" ' +
